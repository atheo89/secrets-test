...
define build_image
	...
	$(eval BUILD_ARGS := --build-arg BUILD_COMMIT_REF=$(RELEASE) --build-arg IMAGE_REGISTRY=$(IMAGE_REGISTRY), --build-arg USERNAME=$(USERNAME), --build-arg PASSWORD=$(PASSWORD))
	$(if $(3),
		$(eval BASE_IMAGE_NAME := $(IMAGE_REGISTRY):$(3)-$(IMAGE_TAG))
		$(eval BUILD_ARGS := $(BUILD_ARGS) --build-arg BASE_IMAGE=$(BASE_IMAGE_NAME)),
	)
	$(CONTAINER_ENGINE) build $(IMAGE_NAME) $(BUILD_ARGS) $(2)
endef
...
# Build and push jupyter-minimal-ubi9-python-3.9 image to the registry
.PHONY: test-ubi9-python-3.9
test-ubi9-python-3.9: base-ubi9-python-3.9
	$(call image,$@,test/ubi9-python-3.9,$<)
